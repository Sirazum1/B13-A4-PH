...
let jobs = [
  {
    ...
    status: null, // Interview | Rejected | null
  },
  {
    id: 2,
...
function setStatus(id, status) {
  // toggle between Interview and Rejected (allowed anytime)
  jobs = jobs.map((j) => (j.id === id ? { ...j, status } : j));

  // requirement: change the tab and count also
  setActiveTab(status);
}

function deleteJob(id) {
  jobs = jobs.filter((j) => j.id !== id);
  render(); // keep current tab
}
